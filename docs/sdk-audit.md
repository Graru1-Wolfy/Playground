...
| Duck / CTAP logic | **Match** | Grounded unduck (`FinishUnDuck` + `CanUnduck`); air unduck (+20 / −20 Z) |
| Knockback (`simulate_knockback`) | **Match** | `DamageForce()` + radius falloff; ground 5.0 / air 10.0 force scales |
| Rocket weapon data | **Match** | Stock/Original offsets; Mangler charge via `+attack2` |
| `float_mode` | **Correct approach** | Emulates C++ `float` at hot points; zlog re-validation still needed |

**Regression status:** 21 pytest scenarios pass snapshot tests with `float_mode = True`.

**Critical open gaps (block v0.1.0 release):** none remaining for core sim gaps (G1–G3 resolved). zlog validation still deferred to v0.4.0.
...
| Self RJ air scale | `tf_damageforcescale_self_soldier_rj` | `10.0` | `10.0` (`TF_DAMAGE_FORCE_SCALE_SELF_SOLDIER_RJ`) | ✅ |
| Air self-damage scale | `tf_damagescale_self_soldier` | `0.60` | `0.60` (`TF_DAMAGE_SCALE_SELF_SOLDIER`) | ✅ |
...
**Air RJ force scale:** SDK uses `tf_damageforcescale_self_soldier_rj = 10.0` with `DamageForce()`. Sim now ports `damage_force()` and applies `0.60` air self-damage via `TF_DAMAGE_SCALE_SELF_SOLDIER` before force calculation.
...
| `CTFGameMovement::OnDuck` / `OnUnDuck` | `handle_ducking` press/release | ✅ grounded unduck |
...
| SDK | Sim | Status |
|-----|-----|--------|
| `CTFPlayer::ApplyPushFromDamage` | `simulate_knockback` | ✅ |
| `DamageForce(vecSize, damage, scale)` | `damage_force()` | ✅ |
| Bounding box center for direction | `center_pos` + bbox closest point | ✅ |
| Explosion position offset | `explosion_pos[2] -= 10.0` | ✅ |
| Falloff `RemapValClamped` | `radius_damage_at_distance()` | ✅ |
| Max impulse cap | `DAMAGE_FORCE_MAX` (1000) | ✅ |
| `tf_damageforcescale_self_soldier_badrj` (5.0) ground | `TF_DAMAGE_FORCE_SCALE_SELF_SOLDIER_BADRJ` | ✅ |
| `tf_damageforcescale_self_soldier_rj` (10.0) air | `TF_DAMAGE_FORCE_SCALE_SELF_SOLDIER_RJ` | ✅ |
| `tf_damagescale_self_soldier` (0.60) air | `TF_DAMAGE_SCALE_SELF_SOLDIER` | ✅ |
...
| Mangler charged shot | `Mangler` + `Charged_standard_rocket` | ✅ |
...
**Current evidence:** 21 regression snapshots pass with `float_mode=True`. Disabling `float_mode` would likely break walkmove rounding-sensitive scenarios (e.g. example_10 ground jumps).
...
| G2 | `Mangler` / `Charged_standard_rocket` | `CTFParticleCannon` / energy ball | ✅ Fixed — `+attack2` charge, 1.33× RJ radius, half falloff |
| G3 | `simulate_knockback` | `ApplyPushFromDamage` + `DamageForce` | ✅ Fixed — SDK falloff, force scales 5.0/10.0, 0.60 air damage |
...
| SDK `tf_player.cpp` knockback audit | ✅ Done | `damage_force` + radius falloff |
| No unresolved **critical** mismatches | ✅ Done | G1–G3 resolved |
...
1. **M5 — Unduck timing:** zlog or in-game test to confirm `0.2` vs `0.3`.
2. **v0.4.0:** Run bcheck zlog pipeline on 64-bit TF2 to classify sim-fix vs empirical-only divergences.
3. **v0.2.0:** Port bcheck analytical core to TypeScript.
...
|---------|------|---------|
| `1.0.2` | 2026-07-07 | SDK `DamageForce` knockback + Mangler charged shot (G2, G3) |
